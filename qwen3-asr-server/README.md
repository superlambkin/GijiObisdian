...
> `model/` が空です。**初回の文字起こし時に自動ダウンロード**されるため手動手順は必須ではありませんが、
> 手動で先にダウンロードしておきたい場合は下記の手順を利用できます。

### 0. モデルの自動ダウンロード（推奨）⭐

モデル未配置のまま初回の文字起こしリクエストを送ると、サーバが自動で Hugging Face から
[Daumee/Qwen3-ASR-0.6B-ONNX-CPU](https://huggingface.co/Daumee/Qwen3-ASR-0.6B-ONNX-CPU)
（約 **4.9 GB**、再開可能）を **セントラルキャッシュ `~/.cache/qwen3-asr`** にダウンロードします。

モデルディレクトリの解決順序:

| 優先 | モデル置き場 | 説明 |
|:----:|------|------|
| 1 | 環境変数 `QWEN3_ASR_MODEL_DIR` | 指定があれば最優先で使用 |
| 2 | `~/.cache/qwen3-asr` | 既定の自動ダウンロード先 |
| 3 | `model/`（レガシー） | 既存配置の後方互換。あれば再ダウンロードしない |

> 💡 サーバは **起動時にモデルをロードしない** ため、`GET /health` はモデル未配置でも即座に `200` を返します。
> モデルは初回の文字起こしリクエスト時に自動ダウンロード＆ロードされます。
...
サーバはモデル未配置でも **即座に起動** します（モデルは初回転写時に遅延ロード）。
以下のログが出れば起動完了です。
...
- **モデルロード時間**: モデルは **初回の文字起こしリクエスト時** に遅延ロードされます。
  サーバ起動は即座に完了し、`GET /health` はモデル未配置でも 200 を返します。
  初回リクエスト時のみ、モデルロード（不足時は自動ダウンロード）で数秒〜十数秒かかります。
